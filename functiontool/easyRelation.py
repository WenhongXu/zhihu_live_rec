...
def new(dbname,typ):
    ...
    c = conn.cursor()
    c.execute('CREATE TABLE id2token(ID CHAR(100) PRIMARY KEY  NOT NULL,TOKEN  '+typ+'  NOT NULL,ISPROCESS int)')
    conn.commit()
    conn.close()
import sqlite3
...
class easyRelation():
    ...
    def insertID_TOKEN(self, uid, token):
        '''
        插入一个id和token的对应关系
        '''
        try:
            self.c.execute(
                'INSERT INTO id2token (ID,TOKEN) VALUES (\'{0}\',\'{1}\')'.format(uid, token))
            self.conn.commit()
        except:
            self.conn.close()
    ...
    def insert(self,dic):
        ...
        uid = dic['id']
        url_token = dic['token']
        isprocess = dic['isprocess']
        try:
            self.c.execute(
                'INSERT INTO id2token (ID,TOKEN,ISPROCESS) VALUES (\'{0}\',\'{1}\',\'{2}\')'.format(uid, url_token, isprocess))
            self.conn.commit()
        except:
            self.conn.close()
            self.conn = sqlite3.connect(self.name+'.db')
    ...
    def createindex(self):
        ...
        self.conn.commit()
    def getone(self,isprocess):
        '''
        在未处理的数据中取一条来处理
        '''
        cur = self.c.execute(
            'SELECT TOKEN from id2token WHERE ISPROCESS='+str(isprocess)+' LIMIT 1')
        c=None
        for row in cur:
            c=row[0]
            break
    ...
    def okone(self,token,isprocess=1):
        '''
        标识该记录已经被处理过
        '''
        self.c.execute('update id2token set ISPROCESS='+str(isprocess)+' where TOKEN = \'' + token + '\'')
        self.conn.commit()
    ...
    def getResult(self, key, mood='VIA_TOKEN'):
        ...
        if mood == 'VIA_TOKEN':
            cur = self.c.execute(
                'SELECT ID,ISPROCESS from id2token WHERE TOKEN=\'' + str(key)+ '\'')
            for row in cur:
                return row[0]
        elif mood == 'VIA_ID':
            cur = self.c.execute(
                'SELECT TOKEN,ISPROCESS from id2token WHERE ID=\'' + str(key) + '\'')
            for row in cur:
                return row[0]
